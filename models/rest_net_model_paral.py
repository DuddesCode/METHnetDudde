...
class Bottleneck_Baseline(nn.Module):
    ...
    def forward(self, x):
        device  = x.device
        self.set_Bottleneck_device(device)
        residual = x
        ...
        #self.set_Bottleneck_device('cuda:0')

        return out
    
    def set_Bottleneck_device(self, device):
...
class ResNet_Baseline(nn.Module):
    ...
    def __init__(self, block, layers):
        ...
        self.layer1 = self._make_layer(block, 64, layers[0]).to('cuda:1')
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2).to('cuda:2')
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2).to('cuda:3')
    ...
    def set_ResNet_Baseline_device(self, device):
        self.layer1.to(device)
        self.layer2.to(device)
        self.layer3.to(device)

def resnet50_baseline_paral(pretrained=False):
